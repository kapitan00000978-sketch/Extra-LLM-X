...
| **Cerebras Cloud** | `llama-3.3-70b`, `llama3.1-8b` | 2,000+ tok/s dunyodagi eng tezkor server |
...
* **`extra/auto-free`** — Tizimdagi eng sifatli va tezkor bepul modelga yo'naltiradi (Groq $\to$ SambaNova $\to$ Gemini Flash $\to$ OpenRouter $\to$ Cerebras).
...
* **`extra/free-reasoning`** — Chuqur fikrlash, matematika va algoritmik tahlil (DeepSeek-R1 SambaNova $\to$ Groq $\to$ OpenRouter).
...
Tizim ishonchliligini tekshirish:
```bash
npm test
```
